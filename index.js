...
async function get_settings() {
    if (url_get.searchParams.get('id') || url_get.searchParams.get('shikimori_id')) {
        ...
        e = await httpGet(url_get.searchParams.get('shikimori_id') ?
            `https://server.dygdyg.ru/api/animes/${url_get.searchParams.get('shikimori_id')}` :
            `https://server.dygdyg.ru/api/animes/${url_get.searchParams.get('id')}`
        )
        if (e.code == "404") {
            url_get.searchParams.delete("id")
            url_get.searchParams.delete("shikimori_id")
...
async function getCalendarSh() {
    ...
    var response = await fetch(`https://server.dygdyg.ru/api/calendar?censored=${base_anime.censored ? base_anime.censored : false}${sh_api.getCookie("sh_access_token") ? "&access_token="+sh_api.getCookie("sh_access_token"):""}`);  //sh_access_token
    const data = await response.json();
